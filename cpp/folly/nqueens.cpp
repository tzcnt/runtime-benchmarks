...
template <size_t N>
folly::coro::Task<int> nqueens(int xMax, std::array<char, N> buf) {
  if (N == xMax) {
    co_return 1;
  }
  ...
  for (int y = 0; y < static_cast<int>(N); ++y) {
    ...
    if (valid) {
      buf[xMax] = q;
      tasks.push_back(nqueens(xMax + 1, buf));
    }
  }

  auto values = co_await folly::coro::collectAllRange(std::move(tasks));
...
int main(int argc, char* argv[]) {
...

  {
    ...
    auto result = folly::coro::blockingWait(
      co_withExecutor(&executor, nqueens(0, buf))); // warmup
    check_answer(result);
  }
  ...
  for (size_t i = 0; i < iter_count; ++i) {
    std::array<char, nqueens_work> buf{};
    auto result =
      folly::coro::blockingWait(co_withExecutor(&executor, nqueens(0, buf)));
    check_answer(result);
    std::printf("output: %d\n", result);
  }
