...
static tmc::task<result> uts(int depth, Node parent) {
  ...
  if (num_children > 0) {
    ...
    // Bounded dispatch: spawn children in fixed-size batches of N so the results
    // come back in a stack std::array<result, N> instead of a heap vector, and
    // the children never need a backing vector either (each child Node lives in
    // its callee coroutine's frame, since uts takes its Node by value).
    //
    // Most nodes have < N children (P99 ~= 20), so they complete in one batch.
    // Wider nodes loop: spawn_many<N> consumes up to N tasks from [it, end) and
    // joins them before the next batch. spawn_many advances an *internal* copy
    // of the iterator, not ours, so we advance `it` by the batch size manually.
    constexpr int N = 9;
    ...
    for (int remaining = num_children; remaining > 0;) {
      ...
      // On a short final batch (remaining < N), spawn_many<N> stops at `end` and
      // leaves the unused array tail default-initialized; we only read [0,batch).
