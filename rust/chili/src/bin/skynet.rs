...
// leaf tasks) and sums their results. chili only offers a binary `join`, so the
...
fn skynet_one(base: u64, depth: u64, scope: &mut Scope<'_>) -> u64 {
    if depth == DEPTH_MAX {
        return base;
    }
    let offset = 10u64.pow((DEPTH_MAX - depth - 1) as u32);
    sum_range(base, offset, depth, 0, 10, scope)
}

// Sum children [lo,hi) of a node whose child 0 has value `base` and whose
// children are spaced `offset` apart, all at `depth`.
fn sum_range(base: u64, offset: u64, depth: u64, lo: u64, hi: u64, scope: &mut Scope<'_>) -> u64 {
    if hi - lo == 1 {
        return skynet_one(base + offset * lo, depth + 1, scope);
    }
    let mid = lo + (hi - lo) / 2;
    let (l, r) = scope.join(
        |s| sum_range(base, offset, depth, lo, mid, s),
        |s| sum_range(base, offset, depth, mid, hi, s),
    );
    l + r
}

fn skynet(expected: u64, scope: &mut Scope<'_>) {
    let count = skynet_one(0, 0, scope);
    if count != expected {
...
fn main() {
    ...
    skynet(expected, &mut scope); // warmup

    let start = Instant::now();
    for _ in 0..ITER_COUNT {
        skynet(expected, &mut scope);
    }
