...
// each on its own monoio Runtime / OS thread.
//
...
//
// monoio is a thread-per-core, io_uring-first runtime: one single-threaded
// Runtime drives all CONNECTION_COUNT connections on a given thread, multiplexed
// as !Send tasks via monoio::spawn. That maps exactly onto io_socket_st (the
// "_st" = single-threaded), whose server and client are each a single event loop.
//
// Unlike the readiness-based runtimes (tokio/smol), monoio's IO is
// completion-based: read/write take *ownership* of the buffer and hand it back
// in a (result, buffer) pair, because io_uring must own the buffer for the
// duration of the kernel operation. We therefore thread one owned Vec through
// each loop instead of borrowing a stack array. The response/request bodies are
// &'static [u8], which monoio accepts directly as a zero-copy write buffer.
//
...
use monoio_bench::peak_memory_usage;
...
async fn server_handler(mut socket: TcpStream) -> usize {
    // One owned read buffer, threaded through the loop. `read` writes into the
    // buffer's spare capacity and returns it with its length set to the byte
    // count; the next iteration reuses the same allocation from offset 0.
    let mut buf = Vec::with_capacity(4096);
    let mut i = 0usize;
    loop {
        let (res, b) = socket.read(buf).await;
        buf = b;
        match res {
            Ok(0) | Err(_) => return i, // client disconnected / error
            Ok(_) => {}
        }
        // &'static [u8] is a valid owned write buffer for monoio; it is handed
        // straight back, so we discard it.
        let (res, _) = socket.write_all(STATIC_RESPONSE.as_bytes()).await;
        if res.is_err() {
            return i;
        }
        i += 1;
...
async fn server(listener: TcpListener, connection_count: usize, request_count: usize) {
    ...
    for _ in 0..connection_count {
        ...
        handlers.push(monoio::spawn(server_handler(socket)));
    }
...
async fn client_handler(count: usize) {
    ...
    let mut done = 0usize;
    for _ in 0..count {
        let (res, _) = s.write_all(STATIC_REQUEST.as_bytes()).await;
        if res.is_err() {
            break;
        }
        let (res, b) = s.read(response_buf).await;
        response_buf = b;
        if res.is_err() {
            break;
        }
    ...
    // Half-close the write side so the server's next read returns EOF (0) and
...
async fn client(connection_count: usize, request_count: usize) {
    ...
    for i in 0..connection_count {
        ...
        clients.push(monoio::spawn(client_handler(count)));
    }
    for c in clients {
        c.await;
    }
...
// A single-threaded, io_uring-backed monoio runtime. Each benchmark thread
// builds its own; there is no shared/work-stealing scheduler in monoio.
fn runtime() -> monoio::Runtime<IoUringDriver> {
...
fn main() {
    ...
    // the harness runs this benchmark repeatedly across a thread sweep. The
    // std listener is then adopted inside the server runtime via from_std
    // (monoio registers the fd with the io_uring driver, which requires the
    // runtime context, so the adoption must happen inside block_on).
