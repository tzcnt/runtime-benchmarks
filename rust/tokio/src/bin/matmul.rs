...
// `tokio::spawn`s so that no output tile is written by two tasks at once.
//
...
// disjoint sub-tiles of C, and the owning `Vec`s outlive `block_on`.
...
fn matmul(m: Mat) -> BoxFut<()> {
    Box::pin(async move {
        ...
        run_group(group1).await;
        run_group(group2).await;
    })
}

async fn run_group(group: [Mat; 4]) {
    let mut handles = Vec::with_capacity(4);
    for m in group {
        handles.push(tokio::spawn(matmul(m)));
    }
    for h in handles {
        h.await.unwrap();
    }
...
fn run_matmul(rt: &tokio::runtime::Runtime, n: usize) -> Vec<i32> {
    let a = vec![1i32; n * n];
    let b = vec![1i32; n * n];
    ...
    rt.block_on(async move { tokio::spawn(matmul(root)).await.unwrap() });
    c
}
...
fn main() {
    ...
    let _ = run_matmul(&rt, n); // warmup

    println!("runs:");
    let start = Instant::now();
    let result = run_matmul(&rt, n);
