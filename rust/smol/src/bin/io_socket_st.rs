...
// each on its own smol `LocalExecutor` / OS thread.
//
...
//
// Each thread drives its executor with `smol::block_on` (async-io's
// reactor-aware block_on), so socket readiness is polled on the benchmark
// threads themselves rather than async-io's fallback "async-io" helper thread.
//
...
use smol_bench::peak_memory_usage;
...
async fn server(
    ...
    for _ in 0..connection_count {
        ...
        handlers.push(ex.spawn(server_handler(socket)));
    }
...
async fn client(ex: &LocalExecutor<'_>, connection_count: usize, request_count: usize) {
    ...
    for i in 0..connection_count {
        ...
        clients.push(ex.spawn(client_handler(count)));
    }
    for c in clients {
        c.await;
    }
...
fn main() {
    ...
    let server_thread = std::thread::spawn(move || {
        ...
        smol::block_on(ex.run(async {
            let listener = TcpListener::try_from(std_listener).expect("try_from");
            server(&ex, listener, connection_count, request_count).await;
        }));
    });
    ...
    {
        let ex = LocalExecutor::new();
        smol::block_on(ex.run(client(&ex, connection_count, request_count)));
    }
