#!/usr/bin/env bash
# Builds every smol benchmark and stages the executables into ./build so that
...
# The runner may request a specific smol version via this env var (used by the
# single-runtime / compare modes). Treat it as a crates.io version requirement.
if [ -n "${RUNTIME_BENCHMARKS_LIBRARY_REF:-}" ]; then
  echo "Pinning smol to ${RUNTIME_BENCHMARKS_LIBRARY_REF}"
  cargo update -p smol --precise "${RUNTIME_BENCHMARKS_LIBRARY_REF}"
fi
