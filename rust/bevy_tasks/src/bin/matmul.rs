...
// `pool.spawn`s so that no output tile is written by two tasks at once.
//
...
// disjoint sub-tiles of C, and the owning `Vec`s outlive the `block_on`.
...
fn matmul(pool: &'static TaskPool, m: Mat) -> BoxFut<()> {
    Box::pin(async move {
        if m.n <= 32 {
        ...
        run_group(pool, group1).await;
        run_group(pool, group2).await;
    })
}

async fn run_group(pool: &'static TaskPool, group: [Mat; 4]) {
    let handles: Vec<_> = group.into_iter().map(|m| pool.spawn(matmul(pool, m))).collect();
    for h in handles {
        h.await;
    }
}

fn run_matmul(pool: &'static TaskPool, n: usize) -> Vec<i32> {
    let a = vec![1i32; n * n];
    let b = vec![1i32; n * n];
    ...
    block_on(pool.spawn(matmul(pool, root)));
    c
}
...
fn main() {
    ...
    let _ = run_matmul(pool, n); // warmup

    println!("runs:");
    let start = Instant::now();
    let result = run_matmul(pool, n);
