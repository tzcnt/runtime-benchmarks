...
- [Paralight](https://github.com/gendx/paralight) - no nested parallelism; single-level fork-join only.
- [ForkUnion](https://github.com/ashvardanian/ForkUnion) - no nested parallelism; single-level fork-join only.
- [orx-parallel](https://github.com/orxfun/orx-parallel) - nested parallelism creates worker threads per task for child levels.
- [trash_parallelism](https://crates.io/crates/trash_parallelism) - library is incomplete; parallel functions are executed sequentially.
- [YACLib](https://github.com/YACLib/YACLib) - CPU-only thread pool with single mutex-locked FIFO queue. Performance is on par with I/O FIFO libraries, without providing I/O.
