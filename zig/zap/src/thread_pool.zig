// Vendored port of kprotty/zap's ThreadPool (the "blog" branch) to Zig 0.16.
//
// Upstream: https://github.com/kprotty/zap/blob/blog/src/thread_pool.zig
//   (commit 97d4b4dc5317c0a6614582b51a60d892721018d2, MIT licensed)
//
// The upstream source targets a pre-0.10 Zig and no longer compiles, so it is
// vendored here with a mechanical modernization. The scheduling algorithm and
// memory orderings are unchanged; only the surface syntax was updated:
//   - std.atomic.Atomic -> std.atomic.Value ('.Monotonic' -> '.monotonic', etc.)
//   - tryCompareAndSwap/compareAndSwap -> cmpxchgWeak/cmpxchgStrong
//   - loadUnchecked() -> direct '.raw' reads
//   - @bitCast/@intToPtr/@ptrToInt/@fieldParentPtr -> 0.16 single-arg builtins
//   - bare 'fn' fields -> '*const fn' pointers
//   - std.Thread.Futex (removed in 0.16) -> raw Linux futex syscalls
//   - std.atomic.fence (removed) -> an acquire load of the same atomic
...
const Event = struct {
    ...
    fn futexWait(state: *const Atomic(u32), expect: u32) void {
        // std.Thread.Futex was removed in Zig 0.16; use the raw Linux futex.
        // Spurious wakeups are fine: the caller re-checks state in a loop.
        _ = linux.futex_4arg(&state.raw, .{ .cmd = .WAIT, .private = true }, expect, null);
    }

    fn futexWake(state: *const Atomic(u32), max_threads: u32) void {
        _ = linux.futex_3arg(&state.raw, .{ .cmd = .WAKE, .private = true }, max_threads);
    }
    ...
    noinline fn wait(self: *Event) void {
        ...
        while (true) {
            ...
            // (An acquire re-load replaces the std.atomic.fence(.Acquire) that 0.16 removed.)
            if (state == SHUTDOWN) {
    ...
    fn shutdown(self: *Event) void {
        return self.wake(SHUTDOWN, std.math.maxInt(i32));
    }
